...
export default function OncallTable({
  ...
  const getOncalls = () => {
    ...
    if (userIds !== undefined && userIds.length > 0) {
      searchParams.append('user_ids', userIds.join(','));
    } else if (
      escalationPolicyIds !== undefined &&
      escalationPolicyIds.length > 0
    ) {
      searchParams.append('escalation_policy_ids', escalationPolicyIds.join(','));
    } else if (scheduleIds !== undefined && scheduleIds.length > 0) {
      searchParams.append('schedule_ids', scheduleIds.join(','));
    }
    return fetch(
